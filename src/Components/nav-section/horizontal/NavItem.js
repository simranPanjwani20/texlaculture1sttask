...
import PlanBasedGuard from "../../../auth/PlanBasedGuard";
...
const NavItem = forwardRef(({item, depth, open, active, isExternalLink, ...other}, ref) => {
    ...
    const {title, path, icon, info, children, disabled, caption, roles, planAccess} = item;
    ...
    return (
        planAccess ? <PlanBasedGuard roles={roles}>{renderItem()}</PlanBasedGuard> :
            <RoleBasedGuard roles={roles}> {renderItem()} </RoleBasedGuard>
    );
});
