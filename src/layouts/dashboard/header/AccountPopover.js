...
import {PATH_AUTH, PATH_COMPANIES, PATH_DASHBOARD, PATH_OFFERS, PATH_USER} from '../../../routes/paths';
// auth
    ...
    const OPTIONS = [
        {
            ...
            linkTo: PATH_DASHBOARD.mySpace.profile,
        },
    ];
    ...
    return (
        <>
            ...
            <MenuPopover open={openPopover} onClose={handleClosePopover} sx={{mt: -1}}>
                ...
                <MenuItem key='Home' sx={{m: 1}} onClick={() => handleClickItem(PATH_USER.home)}>
                    Home
                </MenuItem>
                {(pathname === PATH_COMPANIES.root || pathname === PATH_OFFERS.offer) ? () => console.log("Company screen") :
                    <Stack sx={{p: 1}}>
                        {OPTIONS.map((option) => (
                        ...
                        {fullProfile?.employeeProfile?.employmentStatus !== "IN_ACTIVE" &&
                            <MenuItem key='Employee Directory'
                                      onClick={() => handleClickItem(PATH_DASHBOARD.mySpace.employeeDirectory)}>
                                Employee Directory
                            </MenuItem>}
                    </Stack>}
                {/* <MenuItem onClick={handleOpen} sx={{ m: 1, mt: -1 }}>
                      {/* <Link variant="subtitle1" underline='hover'  onClick={handleOpen} sx={{ textAlign: 'left', mb: 0, cursor: 'pointer' }} fontWeight='bold'>
                        Contact Us
                      </Link> */}
                {/* Contact Us
                    </MenuItem> */}
