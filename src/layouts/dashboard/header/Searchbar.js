...
import {Autocomplete, Box, ClickAwayListener, InputAdornment, InputBase, Popper, Slide,} from '@mui/material';
...
function Searchbar() {
    ...
    // const reduceItems = NavConfig.map((list) => handleLoop(list.items, list.subheader, companyRoles)).flat();
    const reduceItems = NavConfig.map((list) => handleLoop(list.items, list.subheader, companyAccess)).flat();
...
function handleLoop(array, subheader = '', companyAccess) {
    return array.filter(list => {
        // const hasRole = list?.roles?.filter(role => companyRoles.includes(role)).length > 0;
        const hasRole = list?.roles?.filter(role => companyAccess?.mods?.map(mod => mod.id).includes(role)).length > 0;
        // const hasRole = true;
        return list?.roles == null || hasRole
    }).map(list => ({
            subheader,
            ...list,
            ...(list.children && {
